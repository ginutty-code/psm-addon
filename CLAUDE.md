...
≥2 for errors; the project carries a stable warning baseline (9), so failing on any
...
**All nine remaining are deliberate**, so don't "fix" them without reading this first:
...
- **`SpecialTames.lua` unused argument `text`** (an `OnHyperlinkClick` handler) — a
  Blizzard callback signature; the name documents the contract. (`Panel.lua`'s
  `onResize` handler used to be a tenth here for the same reason; it now reads
  `panel` to re-assert the rail-aware minimum width, so the warning is gone.)
